...
from common import resolve_delayed
...
class DataSet:
    ...
    def common_dataframes(self):
        ...
        #dexa_df = pd.read_excel("data/DXAnooutliers.xlsx", na_values=['#N/A'])
...
class BIADataset(DataSet):
    ...
    def load_data(self):
        ...
        #bia_df = bia_df.merge(super().common_dataframes(), on='SubjectID', how='outer')
        styku_df = StykuDataSet().load_data()
        #bia_df = bia_df.rename(columns={"_BFM_Body_Fat_Mass_": "TOTAL_FAT", "_LBM_Lean_Body_Mass_": "TOTAL_LEAN"})
        bia_df = bia_df.merge(styku_df, on='SubjectID', how='outer')
        bia_df['SubjectID'] = standardize_subject_ids(bia_df['SubjectID'])
        return bia_df
...
class MalePcaPoints(DataSet):
    ...
    def load_data(self):
        pca = pd.read_csv(GITPATH + "data/PC_Weights_Male.csv")
        dxa = pd.read_csv(GITPATH + "data/DXA.csv")
...
class MLPData:
    ...
    # Only used in scoring accuracy in runner, that is, not useful.
    def predict(self, mlp):
        y_predict_scaled = dataframe_with_structure(self.y_scaled, mlp.predict(self.x_scaled))
        y_predict = self.scaler.inverse_transform(y_predict_scaled)
        return y_predict
