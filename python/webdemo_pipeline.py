...
Note: We have cleaned up the data and typically DataSets are imported from DataSets.py, So you DON'T need to do this step for OUR datasets!
However, in this file we have illustrated the pipeline starting directly from the output of the MATLAB 
code (an 'ObjOrganizer' xlsx spreadsheet) so the developer learns how to create Datasets if a researcher uploads a new set of data.
'''
...
data_config_dict = {
    ...
    'target_cnames': ['TOTAL_FAT', 'TOTAL_LEAN'], # this is an example for two targets, you can pick any "cname" to be your target
    'feature_options': {
...
results = run_batch(
    ...
    StykuDataset, # dataset created using MyDataset
    ...
    cv_params=dict(
        n_splits=5,
        scorers=['r2', 'explained_variance']  # options for different scores can be found at https://scikit-learn.org/stable/modules/model_evaluation.html
    ),
