"""Silhouette-fit SMPL-X betas to each subject's contours, in-process. No shell vars."""
...
import fit_silhouette  # noqa: E402
...
for s in subjects:
    ...
    if not fit_silhouette.image_paths(sub):
        print(f"[{s}] missing"); continue
    print(f"=== fit {s} ===", flush=True)
    name = os.path.splitext(os.path.basename(s.replace("*", "all")))[0]
    sys.argv = ["fit_silhouette.py", "--subject", sub, "--out", f"{REPO}/runs/fit_{name}"]
    try:
        fit_silhouette.main()
    except SystemExit:
        pass
...
print("BATCH_FIT_DONE", flush=True)
