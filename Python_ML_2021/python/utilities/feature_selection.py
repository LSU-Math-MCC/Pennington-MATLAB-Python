import numpy as np, pandas as pd, seaborn as sns, utilities.datagrid as dg
import os
...
def build_redundancy_list(df, initial_features, feature_pool=[]):
    '''
        Given some list of :initial_features and a :feature_pool, return a list with the nth element chosen to be the
    least redundant with the n-1 previous elements.
    '''
    if feature_pool == []:
...
def select_and_run_targs(df, feature_cnames, targets, model, upto=10, graph_r2=True):
    dataset = to_DataSet(df)
    tdf = pd.DataFrame()
    for target in targets:
        selections = lists_from_firsts(select_n_best_metric(df, feature_cnames, target, n=upto))
        data_config_dict = {
    ...
    if graph_r2:
        ...
        g = sns.FacetGrid(tdf_plot, col="Target", row='SEX', sharex=False)
        g.map(plt.plot, "Selected", "test_r2", marker=".")
        g.map(plt.plot, "Selected", "train_r2", marker="+", color='orange')
