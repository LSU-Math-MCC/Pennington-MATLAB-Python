classdef Avatar
 ...
 % General description: Avatar finds landmarks and computes lengths, cirsumferences, volumes, SA of an avatar
...
%                         end
                    ...
                    elseif strcmp(varargin{i},'armpits_alt')
                        if (strcmp(varargin{i+1},'on') || (varargin{i+1}==1))
                            armpitAlt = 1;
...
%                 [self.r_arm_template_v,self.r_arm_template_f, self.l_arm_template_v,self.l_arm_template_f] = templateFitting_arm(self);
            ...
            if self.Vol_SA
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                  self.list_of_properties(45:end) = [
                 self.volume.total; self.volume.lArm; self.volume.rArm; self.volume.head; self.volume.trunk; self.volume.lleg; self.volume.rleg;... 
                 self.surfaceArea.total; self.surfaceArea.lArm; self.surfaceArea.rArm; self.surfaceArea.head;...
                 self.surfaceArea.trunk; self.surfaceArea.lleg; self.surfaceArea.rleg];                                
            end 
        ...
        function [newV] = fixOrientation(self)
...
% k9_adj(3)
        ...
        %   leftArmpit  - [x,y,z], left armpit
            ...
            step = (maxv1-minv1)*0.05; % step size
            ...
            % Left side
            sideIdx = self.v(:,1) > (maxv1-minv1)*0.5*0.15;
            faces = getFaces(self.f, sideIdx & notopIdx & nobottomIdx);
            px = maxv1;
            pz = (maxv3-minv3)*0.4 + minv3;
        ...
        %   p - [x,y,z], the armpit
            ...
            n = 45; % Number of angles
            multiplier = 1; % Step multiplier
            angle = linspace(-pi/2,3*pi/2,n);
            ...
            while true
                center = [p(1,1),p(1,2)+step*multiplier];
                for i = 2:n
                    nextp = center + [cos(angle(i)),sin(angle(i))]*step*multiplier;
                    ...
                    if sum(s>0 & t>0 & 1-s-t>0)>0 % check for intersection
                        if i == 2
                            % If i == 2, we may have intersected with the
                            % arm. So go to previous point and reduce step
                            % size with multiplier.
                            multiplier = multiplier*0.75;
                            p(1,:) = p(2,:);
                            break;
                        else
                            nextp = center + [cos(angle(i-1)),sin(angle(i-1))]*step*multiplier;
                            p(2,:) = p(1,:); p(1,:) = nextp;
                            break;
                        end
                    end
                end
                if multiplier <= 0.75^15
                    break;
                end
            end
            p = [p(1,1), NaN, p(1,2)];
...
%                 z_backPoint = z(indx_backPoint);
        ...
        function [rtoe_tip, rheel_tip, ltoe_tip, lheel_tip] = getFeetTips(self)
            ...
            [~,Idx_y_rtoe_tip] = min(v_under_rAnkle(:,2));
            x_rtoe_tip = v_under_rAnkle(Idx_y_rtoe_tip,1);
            y_rtoe_tip = v_under_rAnkle(Idx_y_rtoe_tip,2);
            z_rtoe_tip = v_under_rAnkle(Idx_y_rtoe_tip,3);
            rtoe_tip = [x_rtoe_tip,y_rtoe_tip,z_rtoe_tip];
            
            [~,Idx_y_rheel_tip] = max(v_under_rAnkle(:,2));
            x_rheel_tip = v_under_rAnkle(Idx_y_rheel_tip,1);
            y_rheel_tip = v_under_rAnkle(Idx_y_rheel_tip,2);
            z_rheel_tip = v_under_rAnkle(Idx_y_rheel_tip,3);
            rheel_tip = [x_rheel_tip,y_rheel_tip,z_rheel_tip];
            
            v_under_lAnkle = self.v(self.v(:,3)<self.l_ankle(3),:);
            v_under_lAnkle = v_under_lAnkle(v_under_lAnkle(:,1)>0,:);
            
            [~,Idx_y_ltoe_tip] = min(v_under_lAnkle(:,2));
            x_ltoe_tip = v_under_lAnkle(Idx_y_ltoe_tip,1);
            y_ltoe_tip = v_under_lAnkle(Idx_y_ltoe_tip,2);
            z_ltoe_tip = v_under_lAnkle(Idx_y_ltoe_tip,3);
            ltoe_tip = [x_ltoe_tip,y_ltoe_tip,z_ltoe_tip];
            
            [~,Idx_y_lheel_tip] = max(v_under_lAnkle(:,2));
            x_lheel_tip = v_under_lAnkle(Idx_y_lheel_tip,1);
            y_lheel_tip = v_under_lAnkle(Idx_y_lheel_tip,2);
            z_lheel_tip = v_under_lAnkle(Idx_y_lheel_tip,3);
            lheel_tip = [x_lheel_tip,y_lheel_tip,z_lheel_tip];
        end 
        ...
        function [volume,surface] = getSurfaceAreaAndVolume(self)
            ...
            %[volume.legs, surface.legs] = getSurfaceAreaAndVolumePartial(self,self.legIdx,3,volume.total);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%% 
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
            %%% this is incorrect:
            ...
            volume.trunk = volume.total - volume.lArm - volume.rArm - volume.head - abs(volume.rleg)- abs(volume.lleg);
...
%function bdyEdges = getBoundaryEdges(self,faces)
%     if ~exist('faces','var')
%         faces = self.f;
%     end
    % Find all edges in mesh, note internal edges are repeated
    E = sort([faces(:,1) faces(:,2); faces(:,2) faces(:,3); faces(:,3) faces(:,1)]')';
...
%         if length(faceToRemove)>10000
%             stop = 1;
%         end
%         fprintf('Removing %d faces.\n',length(faceToRemove))
%         disp(faceToRemove)
%         for i = 1:length(faceToRemove)
%             fprintf('Removing face %d.\n',faceToRemove(i))
%         end
...
function f = deleteFaceIntersections(f,v)
    epsilon = 10^-10;
    v0 = v(f(:,1),:);
    v1 = v(f(:,2),:);
    v2 = v(f(:,3),:);
    N = cross(v1-v0,v2-v0,2);
    d = -dot(N,v0,2);
    figure(1);
    range = max([max(v(:,1)),max(v(:,2)),max(v(:,3))])*0.1;
    ...
    for i2 = 1:length(f)
        ...
        for i1 = find(rangeF)'
            ...
            dv10 = dot(N(i2,:),v0(i1,:)) + d(i2);
            dv11 = dot(N(i2,:),v1(i1,:)) + d(i2);
            dv12 = dot(N(i2,:),v2(i1,:)) + d(i2);
            
            dv20 = dot(N(i1,:),v0(i2,:)) + d(i1);
            dv21 = dot(N(i1,:),v1(i2,:)) + d(i1);
            dv22 = dot(N(i1,:),v2(i2,:)) + d(i1);
            ...
            % Check if triangles intersect with plane of other triangle
            if (dv10 * dv11 * dv11 * dv12 <= 0) && (dv20 * dv21 * dv21 * dv22 <= 0)
                planeintersect = true;
                if dv10 * dv11 * dv12 == 0
                    % Say that vertex on plane is on opposite side
                    if dv10 == 0
                        case1 = 0;
                    elseif dv11 == 0
                        case1 = 1;
                    ...
                    if dv10 * dv11 > 0
                        case1 = 2; % v2 of face 1 is on opposite site of v0 and v1
                    elseif dv11 * dv12 > 0
                        case1 = 0;
                    else 
                        case1 = 1;
                ...
                if dv20 * dv%!!!! continue here
                end
                if dv20 * dv21 > 0
                    case2 = 2; % v2 of face 2 is on opposite site of v0 and v1
                elseif dv21 * dv22 > 0
                    case2 = 0;
                else 
                    case2 = 1;
            ...
            if planeintersect
                ...
                if case1 == 0
                    t11 = pv12+(pv10-pv12)*(dv12/(dv12-dv10));
                    t12 = pv10+(pv11-pv10)*(dv10/(dv10-dv11));
                elseif case1 == 1
                    t11 = pv10+(pv11-pv10)*(dv10/(dv10-dv11));
                    t12 = pv11+(pv12-pv11)*(dv11/(dv11-dv12));
                else %case1 == 2
                    t11 = pv11+(pv12-pv11)*(dv11/(dv11-dv12));
                    t12 = pv12+(pv10-pv12)*(dv12/(dv12-dv10));
                end
                
                 if case2 == 0
                    t21 = pv22+(pv20-pv22)*(dv22/(dv22-dv20));
                    t22 = pv20+(pv21-pv20)*(dv20/(dv20-dv21));
                elseif case2 == 1
                    t21 = pv20+(pv21-pv20)*(dv20/(dv20-dv21));
                    t22 = pv21+(pv22-pv21)*(dv21/(dv21-dv22));
                else %case2 == 2
                    t21 = pv21+(pv22-pv21)*(dv21/(dv21-dv22));
...
%                 plotAva(f(i1,:),v,'m')
%                 hold on;
%                 text(v0(i1,1),v0(i1,2),v0(i1,3),'v0','Color','m','FontSize',14)
%                 text(v1(i1,1),v1(i1,2),v1(i1,3),'v1','Color','m','FontSize',14)
%                 text(v2(i1,1),v2(i1,2),v2(i1,3),'v2','Color','m','FontSize',14)
%                 text(v0(i2,1),v0(i2,2),v0(i2,3),'v0','Color','b','FontSize',14)
%                 text(v1(i2,1),v1(i2,2),v1(i2,3),'v1','Color','b','FontSize',14)
%                 text(v2(i2,1),v2(i2,2),v2(i2,3),'v2','Color','b','FontSize',14)
%                 xlabel('x')
%                 ylabel('y')
%                 zlabel('z')
                T1 = sort([t11 t12]);
                ...
                fprintf('Intervals %s: [%.4f %.4f] [%.4f %.4f]\n',str,T1(1),T1(2),T2(1),T2(2))
